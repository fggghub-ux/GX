...
\n\n`;
            ...
            systemPrompt += `输出必须为纯 JSON 数组格式，不要任何多余文本或 markdown 标签。格式：字符串内如需强调词语或俚语，一律使用单引号 ' 包裹（例如 the 'wagyu' course），字符串值内禁止使用双引号 " ，否则破坏 JSON 结构导致解析失败。tags 数组长度严格为2。格式要求：\n
