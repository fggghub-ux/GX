...
\n\n`;
            ...
            systemPrompt += `输出必须为纯 JSON 数组格式，不要任何多余文本或 markdown 标签。格式铁律：字符串内如需强调词语、菜名、品牌名或俚语，一律使用单引号 ' 包裹（例如 the 'wagyu' course），禁止在字符串值内使用双引号 " ，否则破坏 JSON 结构导致解析失败。tags 数组长度严格为2。格式要求：\n
...
[
  {
    ...
    "bgGrad": "CSS渐变背景(例如 linear-gradient(135deg, #0A1128 0%, #C41E5C 100%) 避免饱和度>80%的荧光/卡通色（正红/正绿/亮紫/亮粉），色调偏向高端、夜间模式友好、品牌感强)",
    "tags": ["Tag 1", "Tag 2"],
    "reviews": [
