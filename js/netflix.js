...
class NetflixApp {
    ...
    createDefaultCatalog() {
        const make = (id, title, category, summary, coverUrl, tags = []) => ({
            id,
            title,
            category,
            summary,
        ...
        const items = [
            make('default-night-train', 'Vault Zero', 'Financial Crime', 'A Swiss bank discovers at midnight that dozens of safety deposit boxes have lost their records. No locks were broken, alarms triggered, or entries recorded. The investigation uncovers a fund transfer scheme spanning Zurich and Singapore.', 'assets/netflix/vault.jpg', ['Bank Heist', 'Financial Fraud']),
            make('default-summer-letter', 'Inside Trade', 'Financial Crime', 'You’re a junior Wall Street analyst who uncovers executives laundering billions through shell funds. Exposing them could cost you everything — or your life. Staying silent makes you complicit. Every choice could reshape a financial empire.', 'assets/netflix/inside.jpg', ['Insider Trading', 'Moral Choices']),
            make('default-glass-city', 'Crypto Scam', 'Financial Crime', 'The world’s largest crypto exchange collapses overnight, wiping out millions of investors. As its co-founder, you must choose between fleeing, surrendering, or striking back at the mastermind. The truth proves less immutable than the blockchain.', 'assets/netflix/crypto.jpg', ['Cryptocurrency', 'Ponzi Scheme']),
            make('default-star-academy', 'Ponzi Empire', 'Financial Crime', 'A charismatic wealth advisor lures an entire city’s fortune with promises of huge returns. As his trusted assistant, you realize the fund chain is about to collapse. Do you help sustain the scheme or save innocent investors before it falls apart?', 'assets/netflix/ponzi.jpg', ['Trust Collapse', 'Human Greed']),
            make('default-ancient-promise', 'Market Leak', 'Financial Crime', 'On the eve of a company’s merger, a message leaks hugely valuable insider information. As an SEC investigator, you have 48 hours to identify the source. The suspects include your mentor, your ex, and an untouchable power broker.', 'assets/netflix/market.jpg', ['Insider Trading', 'Gray Areas']),
            make('default-island', 'Foreign Funds', 'Financial Crime', 'Behind a quiet teahouse in the Pearl River Delta operates an underground bank moving hundreds of billions a year. As an undercover officer inside, you’re caught between gang loyalty and the law. One botched transfer puts both sides after you.', 'assets/netflix/foreign.jpg', ['Undercover', 'Organized Crime']),
            make('default-stage', 'Silk Road Funds', 'Financial Crime', 'On a Belt and Road project, funds flow into an underground network through fraudulent trade. As a counselor, you uncover links between executives and a terrorist financing group. Exposing them could trigger a diplomatic crisis.', 'assets/netflix/silk.jpg', ['Diplomatic', 'Investigation'])
        ];
    ...
    normalizeCoverUrl(url, seed = 'netflix') {
        ...
        if (/^(https?:\/\/|data:image\/|blob:|(?:\.\/)?assets\/)/i.test(value)) return value;
