...
(window.u2OnStorageReady || (callback => document.addEventListener('DOMContentLoaded', callback)))(() => {
    ...
    const OFFLINE_SHOW_COT_PANEL = false;
...
function createAttachmentSheet(page) {
        ...
        const buildOfflineThinkingHtml = (reasoning, expanded = false) => {
            if (!OFFLINE_SHOW_COT_PANEL) return '';
        ...
        const renderOfflineThinkingState = (bubble, reasoning, options = {}) => {
            ...
            if (!body) return null;
            if (!OFFLINE_SHOW_COT_PANEL) {
                body.querySelector('[data-offline-thinking]')?.remove();
                return null;
            }
            const rawReasoning = String(reasoning || '').trim();
...
<thinking>`,
            ...
            },
            {
                ...
                enabled: false,
                presetVersion: 3,
                content: `是否按照角色默认语言书写台词；台词是否使用英文直引号或中文弯引号；非中文台词是否在下一行紧跟不加引号的准确中文翻译。`,
                editable: true,
                deletable: false
            },
            {
